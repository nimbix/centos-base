...
WORKDIR /tmp
RUN yum -y install zip unzip xz tar file sudo openssh-server openssh-clients infiniband-diags openmpi perftest libibverbs-utils libmthca libcxgb4 libmlx4 libmlx5 dapl compat-dapl dap.i686 compat-dapl.i686 && yum clean all && unzip nimbix.zip && rm -f nimbix.zip && /tmp/image-common-master/setup-nimbix.sh
